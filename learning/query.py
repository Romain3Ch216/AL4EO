...
class Coreset(Query):
    ...
    def __call__(self, model, pool, train_data):
        ...
        # if representation.shape[-1] > dataset.n_classes:
        #     if self.model_feature == 'pca':
        #         pca = PCA(n_components=dataset.n_classes)
        #         pca.fit(representation)
        #         representation = np.dot(representation, pca.components_.T)
        #     else:
...
#   """
#   def __init__(self, n_px, hyperparams, shuffle_prop, dataset, seed, beta=2,\
...
#     if len(labels) == 0:
#       return 0, np.ones(self.n_classes)/self.n_classes, False, []
#
#     prob = self.get_class_counts(labels) / (len(labels) * 1.0)
#     if 1 in labels or 2 in labels or 3 in labels or 4 in labels:
#       tmp = True
#     else:
#       tmp = False
#     return len(labels), prob, tmp, list(set(labels))
#
#   def get_node_upper_lower_bounds(self, node):
#     n_v, p_v, tmp, labels = self.get_node_class_probabilities(node)
#     # If no observations, return worst possible upper lower bounds
#     if n_v == 0:
#       return np.zeros(len(p_v)), np.ones(len(p_v)), tmp, labels
#     delta = 1. / n_v + np.sqrt(p_v * (1 - p_v) / (1. * n_v))
#     return (np.maximum(p_v - delta, np.zeros(len(p_v))),
#             np.minimum(p_v + delta, np.ones(len(p_v))), tmp, labels)
#
#   def get_node_admissibility(self, node):
#     p_lb, p_up, tmp, _ = self.get_node_upper_lower_bounds(node)
...
#     _, prob, _, _ = self.get_node_class_probabilities(node)
#     error = 1 - prob
...
#       if method == 'empirical':
#         _, v_prob, tmp, labels = self.get_node_class_probabilities(v)
#       else:
#         lower, upper, tmp, labels = self.get_node_upper_lower_bounds(v)
#         if method == 'lower':
#           v_prob = lower
#         elif method == 'upper':
#           v_prob = upper
...
#     return np.array(prob_pruning), np.array(tmps), LABELS
#
#   def get_pruning_impurity(self, y):
#     impurity = []
#     for v in self.pruning:
#       _, prob, _, _ = self.get_node_class_probabilities(v, y)
...
#     selected_nodes = set()
#     weights = self.get_node_leaf_counts(self.pruning)
#     probs, tmp, LABELS = self.get_class_probability_pruning()
#     # pdb.set_trace()
#     tmp = tmp == False
#     tmp = tmp.astype(int)
#     probs = 1 - probs
#     weights = weights * probs * tmp
#     weights = weights / sum(weights)
#     batch = []
#
...
#     return output
#
#   def __call__(self, model, y_pool, y_train):
#     _, y_train = y_train
#     _, y_pool = y_pool
#     # _, y_train = dataset.train_data
#     # _, y_pool = dataset.pool_data()
